  ...
  build(data) {
    ...
    if (data.metadata.format === "pmx") {
      ...
      for (const grantEntry of Object.values(grantEntryMap)) {
        const parentGrantEntry = grantEntry.param?.parentIndex != null ? grantEntryMap[grantEntry.param.parentIndex] ?? rootEntry : rootEntry;
...
class MaterialBuilder {
    ...
    for (let i = 0; i < data.metadata.materialCount; i++) {
      ...
      if (data.metadata.format === "pmd") {
        ...
        if (material.textureIndex !== -1) {
          params.map = this.loadTexture(data.textures[material.textureIndex], textures);
  ...
  isDefaultToonTexture(name) {
    if (name.length !== 10)
      return false;
    return /toon(?:10|0\d)\.bmp/.test(name);
  }
  loadTexture(filePath, textures, params, onProgress, onError) {
    params = params || {};
    let fullPath;
    ...
    if (loader === null) {
      loader = filePath.slice(-4).toLowerCase() === ".tga" ? this.getTGALoader() : this.textureLoader;
    }
    const texture = loader.load(fullPath, (t) => {
      if (params.isToonTexture === true) {
        t.image = this.getRotatedImage(t.image);
